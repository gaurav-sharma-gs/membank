...
1. **AST discovery**
   ...
   - Relationship metadata is written to `atlas/runs/<ts>/raw/com/acme/portfolio/InstitutionalPortfolioFlow.json` (includes method signatures, annotations, referenced services).
...
### 11.6 General Checkpointing Guidelines
...
4. **Phase 3 – Atlas Scan Pipeline (Weeks 5–8)**
   - Implement `atlas.config.json`, detectors, and the `atlas:scan` CLI per Sections 4.4 and 11.
...
6. **Phase 5 – Scale & Hardening (Weeks 11+)**
...
Each task includes explicit creation steps, file paths, and verification instructions so a coding agent can execute independently. Work through the tasks sequentially; after finishing a task, confirm functionality (tests/logs) before moving to the next one.
